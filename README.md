...
## Abstract
This work presents a comprehensive investigation into reconstructing RGB images from intermediate feature representations of frozen pre-trained deep neural networks. We systematically evaluate 44 model configurations comprising 8 encoder architectures (ResNet-34, VGG-16, Vision Transformer, Pyramid Vision Transformer at various extraction layers), 4 decoder strategies, and 3 ensemble fusion approaches on the DIV2K dataset.
Our decoder architectures include: (1) Transposed Convolution Decoder - a simple yet effective design using sequential stride-2 transposed convolutions with batch normalization, (2) Frequency-Aware Decoder - explicitly separating low and high-frequency processing paths with spatial-channel attention, (3) Wavelet Frequency Decoder - predicting four wavelet sub-bands (LL, LH, HL, HH) for multi-resolution reconstruction, and (4) Attention Decoder - integrating transformer blocks with self-attention mechanisms. Ensemble models combine features from multiple architectures using attention-based, concatenation-based, or weighted fusion strategies before decoding.
Contrary to expectations that complex domain-specific decoders would excel, our results demonstrate that the simple Transposed Convolution Decoder consistently outperforms all alternatives across every architecture, achieving the highest reconstruction quality. The critical insight is that transposed convolutions allow unconstrained data-driven learning of optimal upsampling patterns without hand-crafted inductive biases, enabling direct end-to-end optimization of the inverse mapping from compressed features to pixel space.
The best-performing configuration is VGG-16 Block1 + Transposed Convolution Decoder, achieving 17.35 dB PSNR and 0.560 SSIM on the test set. Multi-architecture ensemble fusion with weighted combination marginally improves to 17.64 dB PSNR and 0.586 SSIM, representing only a +1.7% gain despite 4× computational cost. This minimal ensemble advantage reveals significant feature redundancy across CNN architectures and highlights that high spatial resolution (112×112) of early-layer features is more valuable than architectural diversity or semantic richness from deeper layers.
Our findings challenge the assumption that complex, domain-informed decoder architectures are necessary for neural reconstruction tasks. Instead, we demonstrate that architectural simplicity combined with high-resolution feature extraction is the optimal strategy, with transposed convolutions providing sufficient expressiveness to learn reconstruction mappings when given adequate spatial information. The success of simple transposed convolutions over frequency decomposition and wavelet-based approaches suggests that for well-posed inverse problems with sufficient training data, unconstrained learnable parameters outperform hand-engineered inductive biases. These results have important implications for neural network interpretability, feature visualization, and compression applications where reconstruction from intermediate representations is required.
